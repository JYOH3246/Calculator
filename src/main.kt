fun main() {
    ...
    while (true) {
        ...
        // 원하는 항목을 입력받는 Input
        val selectCalc: Int // 선언부 : 선언만 먼저 하고 값은 다른 지점에서 주는 것도 가능
        // try : try문 안에는 예외가 발생하는 지점이 들어가야 한다. 그 지점이 들어가지 않으면 예외가 계속 발생함
        try {
            selectCalc= readLine()!!.toInt()  // 여기서 세팅을
        }
        ...
        when (selectCalc) {
            ...
            // selectCalc에 5가 들어왔을 때
            5 -> {
                println("계산기를 종료합니다.")
                break
            }
            // selectCalc에 다른 값이 들어왔을 때
            // 예외처리를 해서 selectCalc에 Int가 아닌 다른 자료형이 들어왔을 때
